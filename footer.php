

<section class="google-map my-5">
    <div class="container-fluid">
        <div class="row">
        <iframe class="p-0 img-fluid" src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d6867.112028964283!2d76.809266!3d30.618284!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x390fe9ec15cbfd03%3A0x4cb1dcaa577519c8!2sRealty%20Nivesh!5e0!3m2!1sen!2sin!4v1740392163687!5m2!1sen!2sin" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
        </div>
    </div>
</section>

<section>
    <div class="container-fluid contact-session mb-5">
        <h2 class="text-center">CONTACT US</h2>
        
   
        <div class="row justify-content-center text-white bg-dark">

            <div class="col-lg-3 text-center py-3">
                <div class="contact-icon">
                <i class="fa-solid fa-phone-volume"></i>
                </div>

                <p>+91 89680 66698</p>
                <p>+91 99880 10405</p>
            </div>

            <div class="col-lg-3 text-center py-3">
                <div class="contact-icon">
                <i class="fa-solid fa-envelope"></i>
                </div>

                <p>info@realtynivesh.com</p>
            </div>
            <div class="col-lg-3 text-center py-3">
                <div class="contact-icon">
                <i class="fa-solid fa-location-dot"></i>
                </div>

                <p>Showroom No. 12 level 2, UPTOWN insignia PR-7 Airport Road, Zirakpur, Punjab</p>
            </div>
        </div>
   
    </div>
</section>


<section>
    <div class="container-fluid download">
        <div class="row align-items-center">
            <div class="col-lg-4 col-md-12 col-12 py-5 download-content">
                <h2 class="text-white">DOWNLOADS</h2>
                <button  type="button" data-bs-target="#exampleModal" data-bs-toggle="modal"><i class="fa-solid fa-book"></i> Brochure</button>
                <button  type="button" data-bs-target="#exampleModal" data-bs-toggle="modal"><i class="fa-solid fa-flag"></i> Site Plans</button>
                <button  type="button" data-bs-target="#exampleModal" data-bs-toggle="modal"><i class="fa-solid fa-indian-rupee-sign"></i> Price list</button>
            </div>


            <div class="col-lg-7 col-md-12 col-12">
                <img src="img/download-sectionimg.webp" class="img-fluid"/>
            </div>
        </div>
    </div>
</section>

<section id="contact">
    <div class="container-fluid footer">


        <div class="container">
            <div class="row">
                <div class="footer-content col-12">
                    <img src="img/combined-logo.webp" alt="">
                </div>
            </div>
        </div>

        <div class="container footer-menu">
            <div class="row">
                <div class="footer-inner">
                    <ul>
                        <li>ABOUT US</li>
                        <li>PRODUCTS</li>
                        <li>LOCATION MAP</li>
                        <li>PLANS</li>
                        <li>PRICE</li>
                        <li>AMENITIES</li>
                        <li>GALLERY</li>
                        <li>FAQ</li>
                        <li>DOWNLOADS</li>
                        <li>CONTACT US</li>

                    </ul>

                    <div class="contact">
                        <ul>
                            <li><i class="fa-solid fa-phone-volume"></i>+91 89680 66698</li>
                            <li><i class="fa-solid fa-phone-volume"></i>+91 99880 10405</li>

                            <li class="social-link">
                                <ul>
                                    <li><i class="fa-brands fa-instagram"></i></li>
                                    <li><i class="fa-brands fa-facebook-f"></i></li>
                                    <li><i class="fa-brands fa-twitter"></i></li>
                                    <li><i class="fa-brands fa-pinterest-p"></i></li>
                                    <li><i class="fa-brands fa-youtube"></i></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>


        <div class="container footer-menu">
            <div class="row align-items-center">

                <ul class="footer-last-content">
                    <li>SALES PARTNER</li>
                    <li><i class="fa-brands fa-telegram"></i></li>
                    <li>
                        REALTY <span>NIVESH :</span>
                    </li>

                    <li>
                        Showroom No. 12 level 2, Uptown Insignia PR-7 Airport Road, Zirakpur, Punjab
                    </li>
                </ul>

            </div>
        </div>

        <div class="footer-overlay"></div>
</section>


<section>
    <div class="container-fluid footer-last">
        <div class="row align-items-center">
            <div class="col-lg-6 col-12 footer-last-text">
                <ul class="d-flex mb-0">
                    <li>Disclaimer</li>
                    <li>Term & conditions</li>
                    <li>Privacy policy</li>
                </ul>
            </div>
            <div class="col-lg-6 col-12 copywrite">
                <p>© Copyright 2024 Medallion Aurum. Realty Nivesh All Rights Reserved</p>
            </div>
        </div>
    </div>
</section>


<!-- Modal -->
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-lg-6 p-5">
                        <h2 class="py-2 text-secondary">We Are Excited To Meet You</h2>
                        <div class="model-img py-4">
                            <img src="img/logo2.webp" class="py-3" alt="logo2" width="200px" />
                            <img src="img/realtynivesh-logo.webp" class="py-3" alt="logo2" width="200px" />
                        </div>

                    </div>

                    <div class="col-lg-6 p-5">
                        <div class="form-container">
                            <form>
                                <div class="mb-4">
                                    <input type="text" class="form-control" id="name" placeholder="Your Name" required />
                                </div>

                                <div class="mb-4">
                                    <input type="tel" class="form-control" id="phone" placeholder="Your Phone Number*" required />
                                    <small class="text-muted">You’ll receive OTP via SMS</small>
                                </div>

                                <div class="mb-4">
                                    <input type="email" class="form-control" id="email" placeholder="Email Id" required />
                                </div>

                                <div class="mb-3 checkbox-container">
                                    <input type="checkbox" id="agree" checked />
                                    <p>
                                        I agree to receive newsletters, or relevant marketing content
                                        and Medallion Aurum Terms and Conditions
                                    </p>
                                </div>

                                <button type="submit" class="btn btn-dark w-100">Get OTP</button>
                            </form>
                        </div>

                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

</body>
<script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.11.8/dist/umd/popper.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.7.1/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/owl.carousel.min.js"></script>

<script src="css/app.js"></script>
<script src="css/index.js"></script>

<script>
    $(function() {
  // Owl Carousel
  var owl = $(".owl-carousel");
  owl.owlCarousel({
    items: 4,
    margin: 10,
    loop: true,
    autoplay: true,
    autoplayTimeout: 3000,
    responsive: {
                0: {
                    items: 1 
                },
                576: {
                    items: 1
                },

                768: {
                    items: 2
                },              
                992: {
                    items: 4
                },
    }
});
});
</script>

</html>